...
interface HistorialObservacionesAeronaveProps {
  aeronaveId: string;
  matricula?: string;
  onClose: () => void;
}

const HistorialObservacionesAeronave: React.FC<HistorialObservacionesAeronaveProps> = ({
  aeronaveId,
  matricula,
  onClose
}) => {
  const [historial, setHistorial] = useState<IObservacionAeronaveHistorial[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [filtroTipo, setFiltroTipo] = useState<string>('todos');
  const [limite, setLimite] = useState<number>(50);
  const [aeronaveInfo, setAeronaveInfo] = useState<any>(null);
  ...
  const tiposObservacion = [
    ...
    { value: 'horas_actualizadas', label: 'Actualización Horas', icon: ClockUpdateIcon },
  ...
  const cargarHistorial = async () => {
    try {
      ...
      setAeronaveInfo(response.data.aeronave);
      setError(null);
    } catch (err) {
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Aeronave: {matricula || aeronaveInfo?.matricula || 'Cargando...'}
              {aeronaveInfo && (
                <span className="ml-2">
                  {aeronaveInfo.tipo} {aeronaveInfo.modelo}
                </span>
              )}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm mt-1">
                Las observaciones aparecerán aquí cuando se agreguen desde la gestión de horas.
              </p>
            </div>
          ) : (
...
export default HistorialObservacionesAeronave;