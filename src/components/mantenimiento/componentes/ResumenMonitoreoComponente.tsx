...
interface ResumenMonitoreoComponenteProps {
  ...
  className?: string;
  compactMode?: boolean;
  estadosPrecargados?: any[]; // ✅ NUEVO: Recibir estados ya cargados
}
...
const ResumenMonitoreoComponente: React.FC<ResumenMonitoreoComponenteProps> = ({
  ...
  className = '',
  compactMode = false,
  estadosPrecargados // ✅ NUEVO
}) => {
  // ✅ Si hay estados precargados, usarlos directamente (NO hacer fetch)
  if (estadosPrecargados !== undefined) {
    const estadosFiltrados = estadosPrecargados;
  ...
  // ⚠️ FALLBACK: Si no hay datos precargados, usar el hook (solo para compatibilidad)
...
const renderEstados = (estadosFiltrados: any[], compactMode: boolean, className: string) => {
